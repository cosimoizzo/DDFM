...
class MarginalizedUKF(BaseFilter):
    ...
    def __init__(
        ...
        linear_observation_map: Union[tf.Tensor, np.ndarray],  # B, the linear part ε of the observation map
        ...
        self.linear_observation_map = _convert_to_tensor(linear_observation_map, self.dtype)
        ...
        self.wm, self.wc = _compute_weights(self.L, self.lamb, self.n_x, self.alpha, self.beta)
    ...
    def _update(self, x_pred: tf.Tensor, P_pred: tf.Tensor, y: tf.Tensor):
        ...
        tmp_R     = self.observation_covariance * outer_mask + tf.linalg.diag(diag_R * diag_mask)

        # Innovation covariance
        cross = Lh @ P_xeps @ tf.transpose(B_eff)
        Pyy = P_hh + cross + tf.transpose(cross) + B_eff @ P_epseps @ tf.transpose(B_eff) + tmp_R
    ...
    def _get_fillna_from_state_function(self):
        ...
        def map_fn(elem):
            ...
            S_pred = (P_hh + cross + tf.transpose(cross)
