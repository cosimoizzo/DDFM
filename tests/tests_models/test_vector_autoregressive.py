...
class TestVARLayerClosedForm(unittest.TestCase):
    ...
    def test_against_stats_models_var1(self):
        ...
        np.testing.assert_allclose(varlayer_coef.T, sm_coef.squeeze(), rtol=1e-5, atol=1e-5)
        # compare predictions
        np.testing.assert_allclose(var_layer(Z_tf, return_valid_only=True).numpy().squeeze(),
                                   results.fittedvalues, rtol=1e-5, atol=1e-5)
    ...
    def test_against_stats_models_var2(self):
        ...
        varlayer_coef = var_layer.coefficients.numpy()
        # lag 1
        np.testing.assert_allclose(varlayer_coef.T[:, :r], sm_coef[0].squeeze(), rtol=1e-5, atol=1e-5)
        # lag 2
        np.testing.assert_allclose(varlayer_coef.T[:, r:], sm_coef[1].squeeze(), rtol=1e-5, atol=1e-5)
        # compare predictions
        np.testing.assert_allclose(var_layer(Z_tf, return_valid_only=True).numpy().squeeze(),
                                   results.fittedvalues, rtol=1e-5, atol=1e-5)
...
class TestVARAutoencoder(unittest.TestCase):
    ...
    def test_var_1_autoencoder(self):
        ...
        encoded = tf.keras.layers.Dense(r,
                                        activation=None,
                                        kernel_initializer="ones",
                                        bias_initializer='zeros')(inputs)
        encoder = tf.keras.Model(inputs, encoded)
        decoded = tf.keras.layers.Dense(d,
                                        activation=None,
                                        kernel_initializer="ones",
        ...
        for epoch in range(epochs):
            with tf.GradientTape() as tape:
                total_loss, recon_loss, var_loss = var_autoencoder.compute_loss(x_tf, x_tf)
        ...
        self.assertAlmostEqual(autcoef, var_layer.coefficients.numpy()[0, 0], places=3, msg="Autoregressive does not match calculated ones.")
