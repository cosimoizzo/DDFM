...
def untransform_variables_series(data_tr, code, fcst_h):
    """
...
            data_untr.loc[:, i] = data_untr.loc[:, i].fillna(value=0).rolling(fcst_h, min_periods=fcst_h).sum() / fcst_h - data_untr.loc[:, i].fillna(value=0)
        else:
            raise ValueError(f"code {i} - {code[i]} not suported")
    return data_untr
