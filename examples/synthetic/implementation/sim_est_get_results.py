...
def run_sims(this_comb: tuple, n_mc_sim: int = 100) -> None:
    ...
    for n_mc in range(n_mc_sim):
        out = run_single_sim(
            seed=n_mc + 1,
            portion_missings=portion_missings,
            n=n,
            r=r,
    ...
    df_results = pd.DataFrame(
        ...
        columns=[
            ...
            "dfm filtered",
            "dfm elapsed time",
            "dfm code reduced size",
            "dfm code reduced size vs linear f",
            "dfm code reduced size elapsed time",
            "ddfm code",
            "ddfm code reduced size",
            "ddfm code reduced size vs linear f",
            "ddfm elapsed time",
            "ddfm nnlin last neurons",
            "ddfm nnlin code",
            "ddfm nnlin code vs linear f",
            "ddfm nnlin elapsed time",
        ],
    )
...
def run_single_sim(
    ...
    end_time = time.time()
    results_dfm[0] = sim.evaluate(
        res_dyn_fact_mdl.factors_ae.smoothed.values, f_true=sim.f
    )
    results_dfm[1] = sim.evaluate(
        res_dyn_fact_mdl.factors_ae.filtered.values, f_true=sim.f
    )
    ...
    res_dyn_fact_mdl = dyn_fact_mdl.fit(disp=100)
    end_time = time.time()
    results_dfm_reduced_code_size[0] = sim.evaluate(
        res_dyn_fact_mdl.factors_ae.smoothed.values, f_true=sim.f
    )
    results_dfm_reduced_code_size[1] = sim.evaluate(
        res_dyn_fact_mdl.factors_ae.smoothed.values, f_true=sim.linear_f
    )
    ...
    if poly_degree > 1:
        ...
        deep_dyn_fact_mdl.fit(pd.DataFrame(x), build_state_space=False)
        results_ddfm[1] = sim.evaluate(
            np.mean(deep_dyn_fact_mdl.factors_ae, axis=0), f_true=sim.f
        )
        results_ddfm[2] = sim.evaluate(
            np.mean(deep_dyn_fact_mdl.factors_ae, axis=0), f_true=sim.linear_f
        )
