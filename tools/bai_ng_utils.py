...
def staz_variables(data, code):
    # chekc if the dropna is fine - maybe there are mising in the monthly variables, whocares!
    data_tr = data.copy(deep=True)
    for i in code.index:
        if code[i] ==1:
            pass
        elif code[i] == 2:
            data_tr.loc[:,i] = data_tr.loc[:, i].dropna().diff(1)
        elif code[i] == 4:
            data_tr.loc[:,i] = np.log(data_tr.loc[:, i].dropna())
        elif code[i] == 5 or code[i] ==6:
            data_tr.loc[:,i] = (np.log(data_tr.loc[:, i].dropna()).diff(1)) * 100
        elif code[i] == 7:
            data_tr.loc[:,i] = ((data_tr.loc[:,i].dropna()/ data_tr.loc[:,i].dropna().shift(1)) -1) * 100
        else:
...
def preprocess_data(data, transform_code_final, target_name, model_name, staz, 
    ...
    if staz:
        data = staz_variables(data, transform_code_final)
